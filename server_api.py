...
async def analyze_and_match_message(
    ...
    「マッチングの質」と「手紙の数の公平性」という2つの要素を考慮して、総合的に判断してください。優先度は「マッチングの質」が少しだけ高いです。

    選定基準:
    1.  マッチングの質の評価: まず、手紙の全体像（タイトル、内容）と、各受信者の「現在の感情(emotion)」および「希望する手紙の種類(custom)」を比較し、どれだけ合致するかを評価してください。手紙の内容と受信者の希望が全く異なる場合（例：楽しい手紙と、悲しい手紙を希望するユーザー）は、マッチングの質が低いと判断し、候補から外してください。
    2.  公平性の考慮: 「現在の受信数」が少ないユーザーほど、手紙を受け取る優先度が高くなります。
    3.  総合判断: 上記2点を踏まえ、まず「マッチングの質」が一定基準以上（良質または許容範囲）の候補者グループを選び出してください。そのグループの中で、「現在の受信数」が最も少ないユーザーを最終的な受信者として選んでください。もし受信数が同じユーザーが複数いる場合は、その中で最もマッチングの質が高いと判断したユーザーを選んでください。
